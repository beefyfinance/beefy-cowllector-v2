...
type LastHarvestReport = z.infer<typeof LastHarvestReportSchema>;
...
lastHarvestReportApi.openapi(lastHarvestReportsRoute, async (c) => {
    ...
    try {
        ...
        return c.json({ data: result.rows } as LastHarvestReportResponse, 200);
    } catch (error) {
...
lastHarvestReportApi.openapi(lastHarvestReportByVaultRoute, async (c) => {
    ...
    try {
        ...
        return c.json(result.rows[0] as LastHarvestReport, 200);
    } catch (error) {
        console.error('Database query failed:', error);
        return c.json({ error: 'Database query failed' }, 500);
    }
