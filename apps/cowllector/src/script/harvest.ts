...
import { getMergedReportAsyncStatus, getReportAsyncStatus, getReportAsyncStatusCounts, type ReportAsyncStatusContext } from '../lib/report-error-status';
                ...
                .map(async ([chain, vaults]) => {
                    ...
                    for (const item of report.details) {
                        ...
                        item.summary = {
                            ...
                            status: getMergedReportAsyncStatus({ chain, vault: item.vault }, [item.simulation, item.decision, item.transaction]),
                            estimatedProfitWei: item.transaction?.status === 'fulfilled' ? item.transaction?.value.estimatedProfitWei : 0n,
                            discordMessage: null,
                        ...
                        if (item.transaction?.status === 'fulfilled') {
                            item.summary.discordTransactionLink = getTransactionDiscordMessageLink(chain, item.transaction.value.transactionHash);
                        }
                    }
                    ...
                    report.summary = {
                        ...
                        aggregatedProfitWei:
                            asyncResultGet(report.collectorBalanceAfter, (ba) =>
                                asyncResultGet(report.collectorBalanceBefore, (bb) => ba.aggregatedBalanceWei - bb.aggregatedBalanceWei)
                            ) || 0n,
                        nativeGasUsedWei:
                            asyncResultGet(report.collectorBalanceAfter, (ba) =>
                        ...
                        wnativeProfitWei:
                            asyncResultGet(report.collectorBalanceAfter, (ba) =>
                                asyncResultGet(report.collectorBalanceBefore, (bb) => ba.wnativeBalanceWei - bb.wnativeBalanceWei)
                            ) || 0n,
