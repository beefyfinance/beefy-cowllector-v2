...
export function extractHarvestReportItemErrorDiscordMessageDetails(chain: Chain, stratReport: HarvestReportItem): string | null {
        ...
        if (stratReport.decision.value.level === 'error') {
            const errorMsg =
                ...
                (stratReport.decision.value.notHarvestingReason === 'harvest would fail'
                    ? ` (block: ${stratReport.decision.value.blockNumber.toString()}, data: ${stratReport.decision.value.harvestReturnData})`
                    : '');
            return `- decision 🔥 ${links}: ${errorMsg}`;
        }
...
export function getVaultDiscordMessageLink(_chain: Chain, vault: BeefyVault): string {
