...
export async function notifyHarvestReport(report: HarvestReport, db_raw_report_id: number | null) {
    ...
    const rolePing =
        (report.summary.statuses.error > 0 || report.summary.statuses.warning > 0 || DISCORD_NOTIFY_UNEVENTFUL_HARVEST) && DISCORD_PING_ROLE_IDS_ON_ERROR.length
...
${rolePing}`),
    ...
    try {
        ...
        // biome-ignore lint/suspicious/noExplicitAny: this works because we know the type of the file
...
export async function notifyRevenueBridgeHarvestReport(report: RevenueBridgeHarvestReport, db_raw_report_id: number | null) {
    if (!DISCORD_REPORT_WEBHOOK_URL) {
        logger.warn({
            msg: 'DISCORD_REPORT_WEBHOOK_URL not set, not sending any discord message',
        });
    ...
    if (report.harvestTransaction && report.harvestTransaction.status === 'rejected') {
        errorDetails += `- 🔥 Revenue bridge harvest transaction failed: ${extractErrorMessage(report.harvestTransaction)}\n`;
    }
