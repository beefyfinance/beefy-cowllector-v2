...
function createMockVault(options: { tvlUsd: number; isClmVault?: boolean; isClmManager?: boolean }): BeefyVault {
    return {
        id: 'test-vault',
        eol: false,
        chain: 'ethereum',
...
describe('getHarvestTimeBucket', () => {
    ...
    describe('CLM manager harvest time requirements', () => {
        ...
        it('should not affect CLM vaults that are not managers', () => {
            const rpcConfig = createMockRpcConfig([], [{ minTvlThresholdUsd: 200, targetTimeBetweenHarvestsMs: 1800000 }]);
