...
import { type RevenueBridgeHarvestReport, reportOnSingleRevenueBridgeHarvestAsyncCall } from './revenue-bridge-harvest-report';
...
export async function revenueBridgeHarvestChain({ report, chain }: { report: RevenueBridgeHarvestReport; chain: Chain }) {
    ...
    const item = await reportOnSingleRevenueBridgeHarvestAsyncCall({ report }, 'collectorBalanceBefore', () => fetchCollectorBalance({ chain }));
    ...
    try {
        await reportOnSingleRevenueBridgeHarvestAsyncCall({ report }, 'collectorBalanceAfter', () => fetchCollectorBalance({ chain }));
    } catch (e) {
        logger.error({
