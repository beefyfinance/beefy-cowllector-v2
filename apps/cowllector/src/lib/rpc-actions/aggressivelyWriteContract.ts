...
export type AggressivelyWriteContractParameters<
    ...
    args extends ContractFunctionArgs<abi, 'nonpayable' | 'payable', functionName> = ContractFunctionArgs<abi, 'nonpayable' | 'payable', functionName>,
...
export type AggressivelyWriteContractReturnType<
    ...
    ///
    minimizedAbi extends Abi = readonly [ExtractAbiFunctionForArgs<abi extends Abi ? abi : Abi, 'nonpayable' | 'payable', functionName, args>],
    resolvedAccount extends Account | undefined = accountOverride extends Account | Address ? ParseAccount<accountOverride> : account,
> = {
    simulation: SimulateContractReturnType<abi, functionName, args, chain, account, chainOverride, accountOverride, minimizedAbi, resolvedAccount>['result'];
    transactionHash: Hex;
...
>(
    { chain }: { chain: Chain },
    args: AggressivelyWriteContractParameters<abi, functionName, args, chain, chainOverride, accountOverride, derivedChain>
): Promise<AggressivelyWriteContractReturnType<abi, functionName, args, chain, account, chainOverride, accountOverride, minimizedAbi, resolvedAccount>> {
    ...
    const mint = async () => {
        ...
        try {
            receipt = await Promise.any(allPendingTransactions.map((hash) => publicClient.aggressivelyWaitForTransactionReceipt({ hash })));
        } catch (err) {
        ...
        try {
            ...
            if (i < rpcConfig.transaction.totalTries - 1) {
                ...
                if (gasParams.gasPrice !== undefined) {
                    gasParams.gasPrice = bigintMultiplyFloat(gasParams.gasPrice, rpcConfig.transaction.retryGasMultiplier.gasPrice);
                }
                ...
                if (gasParams.maxFeePerGas !== undefined) {
                    gasParams.maxFeePerGas = bigintMultiplyFloat(gasParams.maxFeePerGas, rpcConfig.transaction.retryGasMultiplier.maxFeePerGas);
                }
                logger.warn({
