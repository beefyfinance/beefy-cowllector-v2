...
import { reportOnSingleUnwrapAsyncCall, type UnwrapReport, type UnwrapReportShouldUnwrapDecision } from './unwrap-report';
...
export async function unwrapChain({ report, chain }: { report: UnwrapReport; chain: Chain }) {
    ...
    const item = await reportOnSingleUnwrapAsyncCall({ report }, 'collectorBalanceBefore', () => fetchCollectorBalance({ chain }));

    // ===============
    // unwrap decision
    // ===============

    const unwrapDecision = await reportOnSingleUnwrapAsyncCall(item, 'unwrapDecision', async (item): Promise<UnwrapReportShouldUnwrapDecision> => {
    ...
    await reportOnSingleUnwrapAsyncCall(unwrapDecision, 'unwrapTransaction', async (item) => {
        ...
        let gasLimit = bigintMultiplyFloat(rawGasEstimation, rpcConfig.unwrap.balanceCheck.minGasInWalletThresholdAsMultiplierOfEstimatedTransactionCost);

        if (rpcConfig.unwrap.setTransactionGasLimit && rpcConfig.unwrap.forceGasLimit !== null) {
            gasLimit = rpcConfig.unwrap.forceGasLimit;
        }
    ...
    try {
        await reportOnSingleUnwrapAsyncCall({ report }, 'collectorBalanceAfter', () => fetchCollectorBalance({ chain }));
    } catch (e) {
        logger.error({
