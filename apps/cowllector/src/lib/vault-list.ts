...
async function fetchVaults(): Promise<BeefyVault[]> {
    ...
    const vaultResponse = await axios.get<ApiBeefyVaultResponse>(`${BEEFY_API_URL}/harvestable-vaults?_cache_buster=${Date.now()}`);
    ...
    const rawTvls: Record<string, number> = {};
    for (const tvlByVault of Object.values(rawTvlByChains)) {
        Object.assign(rawTvls, tvlByVault);
    }
...
export async function getVault(options: { chain: Chain; strategyAddress: Hex }): Promise<BeefyVault | null> {
    ...
    const vault = vaults.find((vault) => vault.chain === options.chain && vault.strategyAddress.toLowerCase() === options.strategyAddress.toLowerCase());
    return vault || null;
}

export async function getVaultsToMonitorByChain(options: { chains: Chain[]; strategyAddress: Hex | null }): Promise<Record<Chain, BeefyVault[]>> {
    const apiVaults = await fetchVaults();
    ...
    const vaults = [...apiVaults, ...EXTRA_VAULTS_TO_MONITOR]
        ...
        .filter((vault) => (options.strategyAddress ? vault.strategyAddress.toLowerCase() === options.strategyAddress.toLowerCase() : true));
