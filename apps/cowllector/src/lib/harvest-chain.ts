...
import { createDefaultHarvestReportItem, type HarvestReport, reportOnMultipleHarvestAsyncCall, reportOnSingleHarvestAsyncCall } from './harvest-report';
...
export async function harvestChain({
    ...
    const successfulSimulations = await reportOnMultipleHarvestAsyncCall(
        ...
        async (item) => {
            ...
            const noParams = VAULT_IDS_WE_NEED_TO_HARVEST_NO_PARAMS.includes(item.vault.id) && rpcConfig.contracts.harvestLens?.kind === 'v3';
    ...
    const shouldHarvestDecisions = await reportOnMultipleHarvestAsyncCall(successfulSimulations, 'decision', { type: 'parallel' }, async (item) => {
        if (item.vault.eol) {
            return {
                shouldHarvest: false,
                level: 'info',
        ...
        if (blindHarvestChainConfig) {
            ...
            const shouldBlindHarvest = truncatedDate.getTime() % (blindHarvestChainConfig.blindHarvestEveryXHours * 60 * 60 * 1000) === 0;
        ...
        if (VAULT_IDS_WE_SHOULD_BLIND_HARVEST.includes(item.vault.id)) {
            ...
            truncatedDate.setSeconds(0);
            truncatedDate.setMinutes(0);
            const shouldBlindHarvest = truncatedDate.getTime() % (BLIND_HARVEST_EVERY_X_HOURS * 60 * 60 * 1000) === 0;
            if (shouldBlindHarvest) {
                return {
                    shouldHarvest: true,
                    blindHarvestDate: truncatedDate,
        ...
        if (item.simulation.harvestWillSucceed === false) {
            const raisedNotCalmError = item.simulation.harvestResultData.toLocaleLowerCase().startsWith('0x26c87876');
            ...
            if (raisedNotCalmError) {
                const isError = item.simulation.hoursSinceLastHarvest > SILENCE_NOT_CALM_ERRORS_FOR_HOURS * 60 * 60 * 1000;
                return {
                    shouldHarvest: false,
        ...
        if (
            ...
            return {
                shouldHarvest: false,
                level: item.simulation.hoursSinceLastHarvest > rpcConfig.alerting.networkCongestionWaitInDays * 24 ? 'error' : 'warning',
        ...
        if (rpcConfig.transaction.maxGasPricePerTransactionWei && item.simulation.gas.gasPrice > rpcConfig.transaction.maxGasPricePerTransactionWei) {
            return {
                shouldHarvest: false,
                level: item.simulation.hoursSinceLastHarvest > rpcConfig.alerting.networkCongestionWaitInDays * 24 ? 'error' : 'warning',
        ...
        if (!VAULT_IDS_WE_KNOW_HAVE_REWARDS_BUT_IS_NOT_TELLING_US.includes(item.vault.id) && item.simulation.estimatedCallRewardsWei === 0n) {
            ...
            if (
                (SLOW_REWARD_WAIT_IN_HOURS.vault[item.vault.id] && item.simulation.hoursSinceLastHarvest < SLOW_REWARD_WAIT_IN_HOURS.vault[item.vault.id]) ||
                (SLOW_REWARD_WAIT_IN_HOURS.platform[item.vault.platformId] &&
                    item.simulation.hoursSinceLastHarvest < SLOW_REWARD_WAIT_IN_HOURS.platform[item.vault.platformId]) ||
                (item.vault.strategyTypeId &&
                    SLOW_REWARD_WAIT_IN_HOURS.strategyTypeId[item.vault.strategyTypeId] &&
                    item.simulation.hoursSinceLastHarvest < SLOW_REWARD_WAIT_IN_HOURS.strategyTypeId[item.vault.strategyTypeId])
            ) {
                return {
                    shouldHarvest: false,
                    level: 'notice',
        ...
        if (VAULT_IDS_WE_NEED_TO_HARVEST_NO_PARAMS.includes(item.vault.id) && rpcConfig.contracts.harvestLens?.kind !== 'v3') {
            return {
                shouldHarvest: false,
                level: 'error',
    ...
    await reportOnMultipleHarvestAsyncCall(stratsToBeHarvested, 'transaction', { type: 'sequential' }, async (item) => {
        ...
        if (!VAULT_IDS_WE_SHOULD_BLIND_HARVEST.includes(item.vault.id) && rpcConfig.harvest.setTransactionGasLimit) {
            harvestParams = {
                strategyAddress: item.vault.strategyAddress,
                transactionCostEstimationWei: item.simulation.gas.transactionCostEstimationWei,
                transactionGasLimit: bigintMultiplyFloat(item.simulation.gas.rawGasAmountEstimation, rpcConfig.harvest.balanceCheck.gasLimitMultiplier),
                noParams,
            };
        }
    ...
    try {
        await reportOnSingleHarvestAsyncCall({ report }, 'collectorBalanceAfter', () => fetchCollectorBalance({ chain }));
    } catch (e) {
        logger.error({
