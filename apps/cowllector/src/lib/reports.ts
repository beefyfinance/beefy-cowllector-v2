...
export type AVal<TReport, TReportKey> =
    TReportKey extends AKey<TReport>
 ...
 */
export async function reportOnSingleAsyncCall<TReport, TKey extends AKey<TReport>, TItem extends AItem<TReport>, TVal extends AVal<TReport, TKey>>(
    item: TItem,
    reportKey: TKey,
    make: (item: TItem) => Promise<TVal>
): Promise<TItem & { [k in TKey]: TVal }> {
 ...
 * - properly typing the result
 */
export async function reportOnMultipleAsyncCall<TReport, TKey extends AKey<TReport>, TItem extends AItem<TReport>, TVal extends AVal<TReport, TKey>>(
    items: TItem[],
    reportKey: TKey,
    mode: RunMode,
    make: (item: TItem) => Promise<TVal>
 ...
 */
const SECRET_REGEXS = CENSOR_SECRETS_FROM_REPORTS.map((secret) => new RegExp(secret.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'));
