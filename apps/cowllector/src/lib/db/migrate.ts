    ...
    */
    ...
    await db_query(`
        ...
        CREATE OR REPLACE VIEW last_harvest_report_by_chain AS (
          (${allChainIds.map((chain) => `SELECT * FROM raw_harvest_report WHERE chain = '${chain}' ORDER BY datetime DESC LIMIT 1`).join(') UNION ALL (')})
        );

        drop view if exists last_unwrap_report_by_chain cascade;
        -- this is the most efficient top-k query
        CREATE OR REPLACE VIEW last_unwrap_report_by_chain AS (
          (${allChainIds.map((chain) => `SELECT * FROM raw_unwrap_report WHERE chain = '${chain}' ORDER BY datetime DESC LIMIT 1`).join(') UNION ALL (')})
        );
        ...
        CREATE OR REPLACE VIEW last_revenue_bridge_harvest_report_by_chain AS (
          (${allChainIds
              .map((chain) => `SELECT * FROM raw_revenue_bridge_harvest_report WHERE chain = '${chain}' ORDER BY datetime DESC LIMIT 1`)
              .join(') UNION ALL (')})
        );
    `);
    ...
    `,
        [
            allChainIds.map((c) => [
                ...
                RPC_CONFIG[c].revenueBridgeHarvest.balanceCheck.minGasInWalletThresholdAsMultiplierOfEstimatedTransactionCost,
    ...
    await db_query(`
        ...
        CREATE OR REPLACE VIEW last_harvest_run_by_chain AS (
          (${allChainIds
              .map((chain) => `SELECT * FROM cowllector_run WHERE chain = '${chain}' and report_type = 'harvest' ORDER BY datetime DESC LIMIT 1`)
              .join(') UNION ALL (')})
        );
        ...
        CREATE OR REPLACE VIEW last_unwrap_run_by_chain AS (
          (${allChainIds
              .map((chain) => `SELECT * FROM cowllector_run WHERE chain = '${chain}' and report_type = 'unwrap' ORDER BY datetime DESC LIMIT 1`)
              .join(') UNION ALL (')})
        );
        ...
        CREATE OR REPLACE VIEW last_revenue_bridge_harvest_run_by_chain AS (
          (${allChainIds
              .map((chain) => `SELECT * FROM cowllector_run WHERE chain = '${chain}' and report_type = 'revenue-bridge-harvest' ORDER BY datetime DESC LIMIT 1`)
              .join(') UNION ALL (')})
        );
    `);
