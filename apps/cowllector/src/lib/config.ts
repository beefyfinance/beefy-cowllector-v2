...
export const DISCORD_RATE_LIMIT_MIN_SECONDS_BETWEEN_REQUESTS = Number.parseInt(process.env.DISCORD_RATE_LIMIT_MIN_SECONDS_BETWEEN_REQUESTS || '10', 10);
...
export const DISCORD_PING_ROLE_IDS_ON_ERROR = process.env.DISCORD_PING_ROLE_IDS_ON_ERROR ? process.env.DISCORD_PING_ROLE_IDS_ON_ERROR.split(',') : [];
...
export const DB_REPORTS_DAILY_RETENTION_IN_DAYS = Number.parseInt(process.env.DB_REPORTS_DAILY_RETENTION_IN_DAYS || '30', 10);

export const REPORT_URL_TEMPLATE = process.env.REPORT_URL_TEMPLATE || 'https://localhost/report/{{reportId}}';
export const CENSOR_SECRETS_FROM_REPORTS = process.env.CENSOR_SECRETS_FROM_REPORTS ? process.env.CENSOR_SECRETS_FROM_REPORTS.split(',') : [];
...
const REVENUE_BRIDGE_HARVEST_LIMIT_GAS_AMOUNT_MULTIPLIER = Number.parseFloat(process.env.REVENUE_BRIDGE_HARVEST_LIMIT_GAS_AMOUNT_MULTIPLIER || '1.5');
...
export const CHAINS_WE_SHOULD_BLIND_HARVEST: { chain: Chain; blindHarvestEveryXHours: number }[] = [{ chain: 'fraxtal', blindHarvestEveryXHours: 24 }];
...
const defaultRevenueBridgeHarvestConfig: RpcConfig['revenueBridgeHarvest'] = {
    ...
    setTransactionGasLimit: true,
    balanceCheck: {
        minGasInWalletThresholdAsMultiplierOfEstimatedTransactionCost: REVENUE_BRIDGE_HARVEST_LIMIT_GAS_AMOUNT_MULTIPLIER,
    },
};
...
export const RPC_CONFIG: Record<Chain, RpcConfig> = {
    ...
    monad: {
        ...
        harvest: {
            ...defaultHarvestConfig,
            // monad will consume all gas defined as limit
            // but if we don't, transaction will fail with not enough gas errors
...
export const EXPLORER_CONFIG: Record<Chain, ExplorerConfig> = {
    ...
    plasma: {
        ...
        apiUrl: process.env.PLASMA_EXPLORER_API_URL || 'https://api.routescan.io/v2/network/mainnet/evm/9745/etherscan/api',
    ...
    robinhood: {
        addressLinkTemplate: 'https://robinhoodchain.blockscout.com//address/{{address}}',
        transactionLinkTemplate: 'https://robinhoodchain.blockscout.com/tx/{{hash}}',
