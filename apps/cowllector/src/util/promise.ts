...
export function splitPromiseResultsByStatus<T>(results: PromiseSettledResult<T>[]) {
    const rejected = results
        .filter((result): result is Prettify<Exclude<typeof result, PromiseFulfilledResult<unknown>>> => result.status === 'rejected')
        .map((result) => result.reason);
    const fulfilled = results
        .filter((result): result is Prettify<Exclude<typeof result, PromiseRejectedResult>> => result.status === 'fulfilled')
...
async function runParallelBatches<T, R>(items: T[], batchSize: number, process: (item: T) => Promise<R>): Promise<PromiseSettledResult<R>[]> {
    if (batchSize <= 0) {
    ...
    return new Promise<TData>((resolve, reject) => {
        const attemptRetry = async ({ count = 0 } = {}) => {
            ...
            try {
                ...
                if (count < retryCount && (await shouldRetry({ count, error: err as Error }))) return retry({ error: err as Error });
                reject(err);
            }
        };
        attemptRetry();
